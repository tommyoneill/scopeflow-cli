...
async function handleRefinementStep(input, title, filename, type) {
    let refined;
    let accept = false;
    while (!accept) {
...
export async function handleProblemSpaceStep() {
    ...
    if (!input)
        return;
    await handleRefinementStep(input, 'Problem Space', 'problem.md', 'strategy');
}
export async function handleValuePropositionStep() {
    ...
    if (!input)
        return;
    await handleRefinementStep(input, 'Value Proposition', 'value-proposition.md', 'strategy');
}
export async function handlePrinciplesStep() {
    ...
    if (!input)
        return;
    await handleRefinementStep(input, 'Product Principles', 'principles.md', 'strategy');
}
