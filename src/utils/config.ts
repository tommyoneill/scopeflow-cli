...
type EditorValue = typeof DEFAULT_EDITORS[number]['value'];
...
export async function getConfig(): Promise<EditorConfig> {
  try {
    ...
    if (fs.existsSync(CONFIG_FILE)) {
      try {
        const config = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf-8')) as EditorConfig;
        if (!config.editor) {
    ...
    try {
      ...
      const config: EditorConfig = { editor };
