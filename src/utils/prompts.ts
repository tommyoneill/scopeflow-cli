...
let testInputs: (string | boolean)[] = [];
let currentInputIndex = 0;

export function enableTestMode(inputs: (string | boolean)[]) {
  isTestMode = true;
  testInputs = inputs;
  currentInputIndex = 0;
}
...
export const text = async (options: any) => {
  if (isTestMode) {
...
export const select = async (options: any) => {
  if (isTestMode) {
    const input = testInputs[currentInputIndex++];
    console.log(`\n${options.message}`);
    console.log(`> ${input}`);
