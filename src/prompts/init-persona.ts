...
export async function handlePersonaStep(vision: string) {
  ...
  const nameOptions = await getPersonaNameOptions(input as string, vision);
  ...
  const persona = await refinePersona(input as string, vision, selectedName as string);
